...
def js_disabled():
	package_settings = sublime.load_settings('Accessbility-sublime-package.sublime-settings')
	return package_settings.get('disable_default_js_completions', False)


def global_completions(word=None):
	js_completions = sublime.load_settings('Accessbility-js-completions.sublime-settings')
	if js_disabled():
		return []
	if word:
...
def get(type, project_index):
	...
	for completion in all_defs:
		if completion[0].startswith(type):
			trigger = completion[0].split(':  ')[1].replace('.', '_') + '\tAccessbility'
