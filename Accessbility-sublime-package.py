...
class Accessbility():
	def init(self, isST2):
		...
		self.attributes = []
		self.settings = sublime.load_settings('Accessbility-sublime-package.sublime-settings')
		self.settings_completions = sublime.load_settings('Accessbility-completions.sublime-settings')
		self.settings_js_completions = sublime.load_settings('Accessbility-js-completions.sublime-settings')
	...
	def view_settings(self):
		return self.active_view().settings().get('Accessbility', {})
	...
	def add_indexed_directives(self):
		...
		indexed_attrs = [
			tuple([
				'ngDir_' + self.definitionToDirective(directive) + '\tAccessbility',
...
class AccessbilityEventListener(sublime_plugin.EventListener):
	global ng
...
class AccessbilityDeleteCacheCommand(sublime_plugin.WindowCommand):
	global ng
...
class AccessbilityPruneCacheCommand(sublime_plugin.WindowCommand):
	global ng
...
class AccessbilityFileIndexCommand(sublime_plugin.WindowCommand):
...
class AccessbilityFindCommand(sublime_plugin.WindowCommand):
	...
	def run(self):
		...
		if not self.definition_List:
			ng.active_window().run_command('Accessbility_file_index')
			return
...
class AccessbilityGoToDefinitionCommand(sublime_plugin.WindowCommand):
	global ng
...
class AccessbilityGoToDocumentationCommand(sublime_plugin.WindowCommand):
	global ng
	...
	def run(self):
		...
		webbrowser.open('http://docs.Accessbility.org/api/ng.directive:' + definition)


class AccessbilityThread(threading.Thread):
	global ng
